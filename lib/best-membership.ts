// GENERATED FILE. Do not edit by hand.
// Source: the PICK_IDS array and metadata title of every app/best/<slug>/page.tsx.
// Regenerate: python3 scripts/build-best-membership.py
//
// Maps a chair id to the /best/ directories that already name it in their
// picks, so a chair page can link into the category pages that answer the
// category-level query the chair page is currently intercepting.

export interface BestMembership {
  slug: string
  label: string
}

export const BEST_MEMBERSHIP: Record<string, BestMembership[]> = {
  "amamedics-hilux-4d": [
    {
      label: "Best Massage Chairs $3,000 to $5,000",
      slug: "3000-to-5000"
    },
    {
      label: "Best Massage Chairs for Athletes and Sports Recovery",
      slug: "athlete-recovery"
    },
    {
      label: "Best Massage Chairs for Gyms and Wellness Centers",
      slug: "gyms-wellness-centers"
    },
    {
      label: "Best Massage Chairs for Lower Back Pain",
      slug: "lower-back-pain"
    },
    {
      label: "Best Massage Chairs for Petite Buyers",
      slug: "petite-buyers"
    },
    {
      label: "Best Massage Chairs for Seniors",
      slug: "seniors"
    },
    {
      label: "Best Massage Chairs for Stress Relief",
      slug: "stress-relief"
    },
    {
      label: "Best Massage Chairs for Tall People",
      slug: "tall-people"
    },
    {
      label: "Best Massage Chairs Under $5,000 (2026): 6 Picks by Use Case",
      slug: "under-5000"
    }
  ],
  "culanta-sl-track": [
    {
      label: "Best Massage Chairs for Hotels and Hospitality",
      slug: "hotels-hospitality"
    }
  ],
  "daiwa-black-panther-supreme-hybrid": [
    {
      label: "Best Premium Massage Chairs Over $5,000",
      slug: "premium"
    },
    {
      label: "Best Massage Chairs for Tall People",
      slug: "tall-people"
    }
  ],
  "daiwa-legacy-4": [
    {
      label: "Best Massage Chairs for Gyms and Wellness Centers",
      slug: "gyms-wellness-centers"
    },
    {
      label: "Best Massage Chairs for Petite Buyers",
      slug: "petite-buyers"
    },
    {
      label: "Best Premium Massage Chairs Over $5,000",
      slug: "premium"
    },
    {
      label: "Best Massage Chairs for Small Spaces",
      slug: "small-spaces"
    },
    {
      label: "Best Massage Chairs for Tall People",
      slug: "tall-people"
    }
  ],
  "daiwa-majesty-2d": [
    {
      label: "Best Massage Chairs for Petite Buyers",
      slug: "petite-buyers"
    },
    {
      label: "Best Massage Chairs for Tall People",
      slug: "tall-people"
    }
  ],
  "daiwa-pegasus-hybrid": [
    {
      label: "Best Premium Massage Chairs Over $5,000",
      slug: "premium"
    }
  ],
  "ergotec-et-180-pluto": [
    {
      label: "Best Heavy-Duty Massage Chairs",
      slug: "heavy-duty"
    },
    {
      label: "Best Massage Chairs for Sciatica",
      slug: "sciatica"
    }
  ],
  "healthrelife-4d-15-mode": [
    {
      label: "Best Massage Chairs for Hotels and Hospitality",
      slug: "hotels-hospitality"
    }
  ],
  "human-touch-laevo-zg": [
    {
      label: "Best Massage Chairs for Senior Living Facilities",
      slug: "senior-living-facilities"
    },
    {
      label: "Best Massage Chairs for Seniors",
      slug: "seniors"
    }
  ],
  "infinity-dynasty-4d": [
    {
      label: "Best Massage Chairs $3,000 to $5,000",
      slug: "3000-to-5000"
    },
    {
      label: "Best Massage Chairs for Senior Living Facilities",
      slug: "senior-living-facilities"
    },
    {
      label: "Best Massage Chairs for Sleep",
      slug: "sleep"
    }
  ],
  "infinity-genesis-max": [
    {
      label: "Best Massage Chairs for Athletes and Sports Recovery",
      slug: "athlete-recovery"
    },
    {
      label: "Best Massage Chairs Under $10,000",
      slug: "under-10000"
    }
  ],
  "inner-balance-jin": [
    {
      label: "Best Massage Chairs for Hotels and Hospitality",
      slug: "hotels-hospitality"
    },
    {
      label: "Best Massage Chairs for Small Spaces",
      slug: "small-spaces"
    },
    {
      label: "Best Massage Chairs Under $2,000",
      slug: "under-2000"
    }
  ],
  "inner-balance-jin-2": [
    {
      label: "Best Heavy-Duty Massage Chairs",
      slug: "heavy-duty"
    },
    {
      label: "Best Massage Chairs for Seniors",
      slug: "seniors"
    },
    {
      label: "Best Massage Chairs for Stress Relief",
      slug: "stress-relief"
    }
  ],
  "jpmedics-kaze-duo": [
    {
      label: "Best Massage Chairs for Small Spaces",
      slug: "small-spaces"
    }
  ],
  "jpmedics-kumo-4d": [
    {
      label: "Best Heavy-Duty Massage Chairs",
      slug: "heavy-duty"
    },
    {
      label: "Best Premium Massage Chairs Over $5,000",
      slug: "premium"
    },
    {
      label: "Best Massage Chairs for Sciatica",
      slug: "sciatica"
    },
    {
      label: "Best Massage Chairs for Senior Living Facilities",
      slug: "senior-living-facilities"
    }
  ],
  "kahuna-dios-7300": [
    {
      label: "Best Massage Chairs for Lower Back Pain",
      slug: "lower-back-pain"
    },
    {
      label: "Best Massage Chairs for Sciatica",
      slug: "sciatica"
    },
    {
      label: "Best Massage Chairs Under $10,000",
      slug: "under-10000"
    }
  ],
  "kahuna-hm-078": [
    {
      label: "Best Massage Chairs $3,000 to $5,000",
      slug: "3000-to-5000"
    },
    {
      label: "Best Massage Chairs for Arthritis",
      slug: "arthritis"
    },
    {
      label: "Best Heavy-Duty Massage Chairs",
      slug: "heavy-duty"
    },
    {
      label: "Best Massage Chairs Under $5,000 (2026): 6 Picks by Use Case",
      slug: "under-5000"
    }
  ],
  "kahuna-hm-5000": [
    {
      label: "Best Massage Chairs for Small Spaces",
      slug: "small-spaces"
    },
    {
      label: "Best Massage Chairs Under $3,000",
      slug: "under-3000"
    }
  ],
  "kahuna-sm-7300s": [
    {
      label: "Best Massage Chairs for Tall People",
      slug: "tall-people"
    }
  ],
  "kanji-4d-shogun-duo": [
    {
      label: "Best Heavy-Duty Massage Chairs",
      slug: "heavy-duty"
    }
  ],
  "kyota-genki-m380": [
    {
      label: "Best Massage Chairs for Gyms and Wellness Centers",
      slug: "gyms-wellness-centers"
    },
    {
      label: "Best Heavy-Duty Massage Chairs",
      slug: "heavy-duty"
    },
    {
      label: "Best Massage Chairs for Sciatica",
      slug: "sciatica"
    },
    {
      label: "Best Massage Chairs for Senior Living Facilities",
      slug: "senior-living-facilities"
    },
    {
      label: "Best Massage Chairs for Seniors",
      slug: "seniors"
    },
    {
      label: "Best Massage Chairs for Tall People",
      slug: "tall-people"
    }
  ],
  "kyota-konbi-m728-dualpro-4d": [
    {
      label: "Best Massage Chairs for Sleep",
      slug: "sleep"
    }
  ],
  "kyota-yugana-m780": [
    {
      label: "Best Massage Chairs for Arthritis",
      slug: "arthritis"
    },
    {
      label: "Best Heavy-Duty Massage Chairs",
      slug: "heavy-duty"
    },
    {
      label: "Best Massage Chairs for Petite Buyers",
      slug: "petite-buyers"
    },
    {
      label: "Best Premium Massage Chairs Over $5,000",
      slug: "premium"
    },
    {
      label: "Best Massage Chairs for Seniors",
      slug: "seniors"
    },
    {
      label: "Best Massage Chairs Under $10,000",
      slug: "under-10000"
    }
  ],
  "luraco-i9-max-plus": [
    {
      label: "Best Heavy-Duty Massage Chairs",
      slug: "heavy-duty"
    },
    {
      label: "Best Massage Chairs for Hotels and Hospitality",
      slug: "hotels-hospitality"
    },
    {
      label: "Best Massage Chairs for Tall People",
      slug: "tall-people"
    }
  ],
  "luraco-theater-sofy": [
    {
      label: "Best Massage Chairs for Petite Buyers",
      slug: "petite-buyers"
    }
  ],
  "medical-breakthrough-6": [
    {
      label: "Best Massage Chairs for Arthritis",
      slug: "arthritis"
    },
    {
      label: "Best Massage Chairs for Seniors",
      slug: "seniors"
    }
  ],
  "ogawa-og6300": [
    {
      label: "Best Massage Chairs for Senior Living Facilities",
      slug: "senior-living-facilities"
    }
  ],
  "ogawa-og6400": [
    {
      label: "Best Massage Chairs for Office Workers and Desk Jobs",
      slug: "office-workers"
    }
  ],
  "ogawa-og8900": [
    {
      label: "Best Massage Chairs for Senior Living Facilities",
      slug: "senior-living-facilities"
    }
  ],
  "ogawa-og8901": [
    {
      label: "Best Massage Chairs for Arthritis",
      slug: "arthritis"
    },
    {
      label: "Best Massage Chairs Under $10,000",
      slug: "under-10000"
    }
  ],
  "osaki-os-pro-4d-duomax": [
    {
      label: "Best Massage Chairs for Gyms and Wellness Centers",
      slug: "gyms-wellness-centers"
    },
    {
      label: "Best Heavy-Duty Massage Chairs",
      slug: "heavy-duty"
    }
  ],
  "osaki-os-pro-maestro-le": [
    {
      label: "Best Massage Chairs for Arthritis",
      slug: "arthritis"
    },
    {
      label: "Best Massage Chairs for Athletes and Sports Recovery",
      slug: "athlete-recovery"
    },
    {
      label: "Best Massage Chairs for Gyms and Wellness Centers",
      slug: "gyms-wellness-centers"
    },
    {
      label: "Best Massage Chairs for Lower Back Pain",
      slug: "lower-back-pain"
    },
    {
      label: "Best Massage Chairs for Neck and Shoulder Pain",
      slug: "neck-shoulders"
    },
    {
      label: "Best Massage Chairs for Office Workers and Desk Jobs",
      slug: "office-workers"
    },
    {
      label: "Best Premium Massage Chairs Over $5,000",
      slug: "premium"
    },
    {
      label: "Best Massage Chairs for Stress Relief",
      slug: "stress-relief"
    },
    {
      label: "Best Massage Chairs Under $10,000",
      slug: "under-10000"
    }
  ],
  "osaki-os-pro-yamato": [
    {
      label: "Best Massage Chairs for Sleep",
      slug: "sleep"
    }
  ],
  "panasonic-maf1": [
    {
      label: "Best Massage Chairs $3,000 to $5,000",
      slug: "3000-to-5000"
    },
    {
      label: "Best Massage Chairs for Neck and Shoulder Pain",
      slug: "neck-shoulders"
    },
    {
      label: "Best Massage Chairs for Petite Buyers",
      slug: "petite-buyers"
    },
    {
      label: "Best Massage Chairs Under $5,000 (2026): 6 Picks by Use Case",
      slug: "under-5000"
    }
  ],
  "panasonic-mak1": [
    {
      label: "Best Massage Chairs for Neck and Shoulder Pain",
      slug: "neck-shoulders"
    }
  ],
  "positive-posture-brio-plus": [
    {
      label: "Best Premium Massage Chairs Over $5,000",
      slug: "premium"
    },
    {
      label: "Best Massage Chairs Under $10,000",
      slug: "under-10000"
    }
  ],
  "positive-posture-brio-sport": [
    {
      label: "Best Massage Chairs for Athletes and Sports Recovery",
      slug: "athlete-recovery"
    },
    {
      label: "Best Massage Chairs for Tall People",
      slug: "tall-people"
    }
  ],
  "relaxe-shiatsu": [
    {
      label: "Best Massage Chairs for Small Spaces",
      slug: "small-spaces"
    },
    {
      label: "Best Massage Chairs for Stress Relief",
      slug: "stress-relief"
    },
    {
      label: "Best Massage Chairs Under $3,000",
      slug: "under-3000"
    },
    {
      label: "Best Massage Chairs Under $5,000 (2026): 6 Picks by Use Case",
      slug: "under-5000"
    }
  ],
  "relaxonchair-jasper": [
    {
      label: "Best Massage Chairs Under $2,000",
      slug: "under-2000"
    }
  ],
  "relaxonchair-mk-v-plus": [
    {
      label: "Best Massage Chairs for Small Spaces",
      slug: "small-spaces"
    }
  ],
  "relaxonchair-yukon-4d": [
    {
      label: "Best Massage Chairs for Athletes and Sports Recovery",
      slug: "athlete-recovery"
    },
    {
      label: "Best Massage Chairs for Office Workers and Desk Jobs",
      slug: "office-workers"
    }
  ],
  "relx-20-mode": [
    {
      label: "Best Massage Chairs for Hotels and Hospitality",
      slug: "hotels-hospitality"
    }
  ],
  "rockertech-bliss": [
    {
      label: "Best Massage Chairs for Arthritis",
      slug: "arthritis"
    }
  ],
  "rockertech-sensation-4d": [
    {
      label: "Best Massage Chairs for Office Workers and Desk Jobs",
      slug: "office-workers"
    }
  ],
  "sharper-image-relieve-3d": [
    {
      label: "Best Massage Chairs $3,000 to $5,000",
      slug: "3000-to-5000"
    },
    {
      label: "Best Massage Chairs Under $5,000 (2026): 6 Picks by Use Case",
      slug: "under-5000"
    }
  ],
  "synca-wellness-circ": [
    {
      label: "Best Massage Chairs for Stress Relief",
      slug: "stress-relief"
    },
    {
      label: "Best Massage Chairs Under $2,000",
      slug: "under-2000"
    },
    {
      label: "Best Massage Chairs Under $3,000",
      slug: "under-3000"
    }
  ],
  "synca-wellness-circ-3": [
    {
      label: "Best Massage Chairs for Lower Back Pain",
      slug: "lower-back-pain"
    },
    {
      label: "Best Massage Chairs for Sciatica",
      slug: "sciatica"
    },
    {
      label: "Best Massage Chairs Under $2,000",
      slug: "under-2000"
    }
  ],
  "synca-wellness-circ-plus": [
    {
      label: "Best Massage Chairs for Hotels and Hospitality",
      slug: "hotels-hospitality"
    },
    {
      label: "Best Massage Chairs for Office Workers and Desk Jobs",
      slug: "office-workers"
    },
    {
      label: "Best Massage Chairs for Small Spaces",
      slug: "small-spaces"
    },
    {
      label: "Best Massage Chairs Under $2,000",
      slug: "under-2000"
    },
    {
      label: "Best Massage Chairs Under $3,000",
      slug: "under-3000"
    }
  ],
  "synca-wellness-kurodo": [
    {
      label: "Best Massage Chairs for Office Workers and Desk Jobs",
      slug: "office-workers"
    }
  ],
  "titan-pro-vigor-4d": [
    {
      label: "Best Massage Chairs for Athletes and Sports Recovery",
      slug: "athlete-recovery"
    },
    {
      label: "Best Massage Chairs for Gyms and Wellness Centers",
      slug: "gyms-wellness-centers"
    },
    {
      label: "Best Massage Chairs for Lower Back Pain",
      slug: "lower-back-pain"
    },
    {
      label: "Best Premium Massage Chairs Over $5,000",
      slug: "premium"
    },
    {
      label: "Best Massage Chairs for Sleep",
      slug: "sleep"
    },
    {
      label: "Best Massage Chairs Under $5,000 (2026): 6 Picks by Use Case",
      slug: "under-5000"
    }
  ]
}

export function bestDirectoriesFor(chairId: string): BestMembership[] {
  return BEST_MEMBERSHIP[chairId] || []
}
